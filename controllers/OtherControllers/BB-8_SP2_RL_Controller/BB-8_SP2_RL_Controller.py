...
class BB8Robot_GoFAST(RobotSupervisorEnv):
    ...
    def intersecting_obstacle_plane(self):
        ...
        obs_upper_x = obs_trans_values[0] + 12/2 + 0.25
        obs_lower_x = obs_trans_values[0] - 12/2 - 0.25
    ...
    def intersecting_goal_plane(self):
        ...
        goal_upper_x = goal_trans_values[0] + 12/2 + 0.25
        goal_lower_x = goal_trans_values[0] - 12/2 - 0.25
    ...
    def get_default_observation(self):
        ...
        rand_x = np.random.uniform(-7.25, 7.25)
        rand_y = np.random.uniform(-9.75,9.75)
    ...
    def get_reward(self, action=None):
        ...
        #print("REWARD: ", reward)
        reward = 0
        #if math.sqrt((self.prev_x-x)**2 + (self.prev_y-y)**2) >= 0.25:
        #    reward += 0.01
        #self.prev_x = x
        #self.prev_y = y
    ...
    def solved(self):
        if len(self.episode_score_list) > 200:  # Over 100 trials thus far
            ...
            if np.sum(self.episode_score_list[-200:]) > 1960:  # Last 100 episodes' scores average value
                print(np.sum(self.episode_score_list[-200:]))
                return True
        return False
...
while not solved and episode_count < episode_limit:
    ...
    if episode_count % 1000 == 0:    
        ...
        agent.save("C:\\Users\\mcgra\\OneDrive\\Documents\\CourseMaterials\\DMU\\FinalProject\\BB-8_Stunts\\SP2_Agents\\sp2_20")
